    ...
    table.insert(tasks, function(cb)
      ...
      MySQL.Async.fetchAll(
        ...
        function(inventory)
          ...
          if(inventory[1]) then
            inv = json.decode(inventory[1].items)
          ...
          --     name      = inv[i].item,
          --     count     = inv[i].count,
          --     label     = ESX.Items[inv[i].item].label,
          --     limit     = ESX.Items[inv[i].item].limit,
          --     usable    = ESX.UsableItemsCallbacks[inv[i].item] ~= nil,
          --     rare      = ESX.Items[inv[i].item].rare,
          --     canRemove = ESX.Items[inv[i].item].canRemove,
          --   })
          -- end
          ...
          if(not inventory[1]) then           
            MySQL.Async.execute(
              'INSERT INTO user_inventory (identifier, items) VALUES (@identifier, @items)',
              {
                ['@identifier'] = player.getIdentifier(),
                ['@items']       = json.encode(userData.inventory)
              }            
            )
          end
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
  ...
  elseif type == 'item_account' then
    ...
    if itemName == 'bank' then
      ...
      TriggerClientEvent('esx:showNotification', _source, 'Impossible')
